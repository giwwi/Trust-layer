...
export async function POST(request: NextRequest) {
  let payload: { text?: string; language?: OutputLanguage; api_key?: string };

  try {
    payload = await request.json();
  } catch (_error) {
    return publicError("invalid_input", "Invalid request for this public demo.", 422);
  }

  const text = normalizeText(payload.text || "");
  const suppliedApiKey = String(payload.api_key || "").trim();
  const language: OutputLanguage = payload.language === "ru" ? "ru" : "en";
  const minChars = numberEnv("TRUST_LAYER_MIN_INPUT_CHARS", 150);
  const publicMaxChars = numberEnv("MAX_INPUT_CHARS", 8000);

  if (!text) {
    return publicError("invalid_input", "Please paste text or upload a supported file before running analysis.", 400);
  }

  if (text.length < minChars) {
    return publicError(
      "invalid_input",
      `Text is too short for this demo. Please provide at least ${minChars} characters of analytical text.`,
      400
    );
  }
  ...
  if (!suppliedApiKey && text.length > publicMaxChars) {
    return publicError(
      "invalid_input",
      `This demo accepts at most ${publicMaxChars} characters. Please shorten the text or use the example.`,
      400
    );
  }
  ...
  if (!apiKey) {
    return publicError("provider_error", "Live AI is not configured for this demo.", 502);
  }

  if (!suppliedApiKey) {
    const limited = applyPublicLimits(request, text.length);
  ...
  try {
    ...
    return publicError(code, "The AI provider could not complete the request.", status);
  }
}

function applyPublicLimits(request: NextRequest, requestLength: number) {
  const day = today();
...
async function callOpenAI(text: string, language: OutputLanguage, apiKey: string, hasOwnKey: boolean) {
  ...
  if (!response.ok) {
    throw new ProviderError(raw, response.status);
  }

  const responseJson = JSON.parse(raw);
  if (responseJson.error) {
    throw new ProviderError(JSON.stringify(responseJson.error), response.status);
  }
...
function publicError(code: string, message: string, status: number) {
  return NextResponse.json({ detail: { code, message } }, { status, headers: corsHeaders });
}
...
function logRequest(requestLength: number, success: boolean) {
  console.info(
    `public_demo_request timestamp=${new Date().toISOString()} request_length=${requestLength} success=${success}`
  );
}
